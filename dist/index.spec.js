...
describe('CastDeviceEmulator', function () {
  ...
  describe('.close()', function () {
    it('should close websocket server properly', function (done) {
      this.timeout(30 * 1000);

      var scenario = require('../examples/scenarios/example.json');

      var emulator1 = new CastDeviceEmulator();
      var emulator2 = new CastDeviceEmulator();
  ...
  describe('WebSocket basic operations', function () {
    it('should respond to websocket client connection', function (done) {
      ...
      var wsc = new WebSocket('ws://localhost:8008/v2/ipc');
      ...
      wsc.on('message', function incoming(message, flags) {
        ...
        var incomingMessageObject = JSON.parse(message);
        var dataProperty = JSON.parse(incomingMessageObject.data);
        var typeProperty = dataProperty.type;

        // Every incoming IPC messages should match expected json-schema
        expect(jsonSchemaValidator.validate(require('../schemas/ipc-message.json'), incomingMessageObject)).to.be.true;
